...
// ============================================================================
// Deprecation aliases — old `skim test` / `skim build` category syntax
// ============================================================================

/// `skim test cargo` (old v2.7 syntax) must succeed (backward compatibility)
/// and print a deprecation warning to stderr.
#[test]
fn test_skim_test_deprecated_category_warns_and_succeeds() {
    let json_input = r#"{"type":"suite","event":"started","test_count":1}
{"type":"test","event":"ok","name":"compat_test","exec_time":0.001}
{"type":"suite","event":"ok","passed":1,"failed":0,"ignored":0,"measured":0,"filtered_out":0,"exec_time":0.001}
"#;

    Command::cargo_bin("skim")
        .unwrap()
        // Old category syntax: `skim test cargo`
        .args(["test", "cargo"])
        // Remove SKIM_PASSTHROUGH so compression is not bypassed inside the child process.
        .env_remove("SKIM_PASSTHROUGH")
        .write_stdin(json_input)
        ...
        // Compression must still work
        .stdout(predicate::str::contains("pass: 1"));
}

/// `skim build cargo` (old v2.7 syntax) must print a deprecation warning to
/// stderr and forward to the cargo build handler (backward compatibility).
///
/// Build commands always spawn the real executable (no stdin path).
