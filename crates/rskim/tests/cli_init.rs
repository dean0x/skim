...
// ============================================================================
// Bare-command migration
// ============================================================================

/// When an existing hook script has the current version string but still uses
/// an old absolute-path format (e.g., `exec "/usr/local/bin/skim" rewrite --hook`),
/// `skim init` must rewrite it to the bare-command format (`exec skim rewrite --hook`)
/// even though the version number has not changed.
///
/// This is the core regression guard for the fix/hook-bare-command branch.
#[test]
fn test_init_migrates_old_absolute_path_format_to_bare_command() {
    let dir = TempDir::new().unwrap();
    let config = dir.path();

    // Set up hooks directory with a hook script in the OLD absolute-path format.
    // The version string matches the current binary version so that version alone
    // would NOT trigger re-generation — only the absence of `exec skim ` (bare
    // command) must trigger the rewrite.
    let hooks_dir = config.join("hooks");
    fs::create_dir_all(&hooks_dir).unwrap();
    let hook_path = hooks_dir.join("skim-rewrite.sh");
    let current_version = env!("CARGO_PKG_VERSION");
    let old_format_content = format!(
        "#!/usr/bin/env bash\n\
         # skim-hook v{version}\n\
    ...
    fs::write(&hook_path, &old_format_content).unwrap();

    // Run `skim init --yes` — should detect the old absolute-path format and
    // rewrite the script even though the version already matches.
    skim_init_cmd(config)
        .args(["--yes"])
        .assert()
        .success()
    ...
    // The script must now use bare `skim` (PATH-resolved, no absolute path).
    let content = fs::read_to_string(&hook_path).unwrap();
    assert!(
        content.contains("exec skim "),
        "Migrated script should use bare 'exec skim' command, got:\n{content}"
    );

    // The old quoted absolute path must be gone.
    assert!(
        !content.contains("\"/usr/local/bin/skim\""),
        "Migrated script must not contain quoted absolute path, got:\n{content}"
    );
    assert!(
        !content.contains("exec \"/"),
        "Migrated script must not have exec with absolute path, got:\n{content}"
    );
}
