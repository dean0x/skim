...
pub(crate) fn parse_impl(output: &CommandOutput) -> ParseResult<InfraResult> {
    ...
    match compress_log(text, &flags) {
        ParseResult::Full(log_result) => ParseResult::Full(log_result_to_infra(log_result)),
        ParseResult::Degraded(log_result, warnings) => {
            ParseResult::Degraded(log_result_to_infra(log_result), warnings)
        }
        ParseResult::Passthrough(raw) => ParseResult::Passthrough(raw),
    }
}

fn log_result_to_infra(log_result: crate::output::canonical::LogResult) -> InfraResult {
    ...
    InfraResult::new("kubectl".to_string(), "logs".to_string(), summary, items)
}
...
mod tests {
    ...
    fn test_logs_structured() {
        let fixture = load_fixture("kubectl_logs.txt");
