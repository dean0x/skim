...
mod tests {
    ...
    // ========================================================================
    // Argv classification: GrepArgs::scan
    //
    // GrepArgs is test-only: production code no longer needs argv classification
    // (parse_impl always passes through).  The struct and its methods live here
    // to document grep flag semantics and guard against future regressions.
    // ========================================================================

    /// Long grep options whose value is the NEXT token (unless `--opt=value`).
