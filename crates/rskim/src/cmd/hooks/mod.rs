...
pub(crate) fn generate_hook_script(version: &str, agent_cli_name: &str) -> String {
    ...
    format!(
        "#!/usr/bin/env bash\n\
         ...
         # The skim binary strips this entry at startup to prevent infinite recursion.\n\
         export PATH=\"$HOME/.skim/bin${{PATH:+:$PATH}}\"\n\
...
mod tests {
    ...
    fn test_generate_hook_script_structure() {
        ...
        // PATH prepend must appear BEFORE the exec line (Step 9).
        assert!(
            script.contains("export PATH=\"$HOME/.skim/bin"),
            "hook script must prepend ~/.skim/bin to PATH"
        );
