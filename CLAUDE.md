...
- `SKIM_DEBUG=1` (or `--debug`) — enables: (1) diagnostic banners on stderr for no-loss raw-fallback paths (see **Stderr notice taxonomy** in Design Constraints below; loss-bearing elision markers and the ADR-008 transparency marker are unconditional and not gated by this variable); (2) the model-facing `additionalContext` advisory on the hook channel when binary drift is detected (`systemMessage` user-facing one-liner fires unconditionally on drift; `additionalContext` is gated because the platform replays it on `--continue`/`--resume`, making its cost permanent — `skim doctor` is the primary diagnosis path). In hook mode the startup provenance line goes to `hook.log`, never stderr (GRANITE #361 Bug 3).
