...
- **Analytics:** token savings persist to `<cache-root>/analytics.db` (SQLite/WAL; default location — relocates with `SKIM_CACHE_DIR`, see Environment Variables), recorded fire-and-forget on background threads. The cache root is `dirs::cache_dir()/skim`, which is **platform-dependent**: `~/Library/Caches/skim/` on macOS, `~/.cache/skim/` on Linux. `~/.cache/skim` does not exist on a macOS dev box — read the real paths off `skim doctor` rather than assuming the XDG one. `--clear-cache` clears only the parser cache, NOT `analytics.db` — use `skim stats --clear` for that. The `AnalyticsStore` trait + `MockStore` make the stats dashboard testable without a real DB.
...
- `SKIM_PASSTHROUGH=1` (or `--passthrough` CLI flag) — bypass compression and exec the real tool with raw argv, at the structural convergence gate in `cmd/dispatch.rs` (~line 1195). **It is not universal — see the two exceptions below; "bypasses all compression" is the intent, not the measured behaviour.** The `--passthrough` flag strips skim-only flags from the forwarded argv so the real tool never sees flags it does not understand. Stripped for all tools: `--show-stats`, `--passthrough`, `--debug` (only when the tool does not own that flag), `--max-lines N`/`=N`, `--tokens N`/`=N`, `--last-lines N`/`=N`, `--line-numbers` (long form only; `-n` is not stripped). Additionally stripped for `git` only: bare `--json` and `--mode`/`--mode=<val>`. Nothing is stripped after a bare `--` end-of-options separator. Exception 1 — `env`/`printenv`: `SKIM_PASSTHROUGH=1` does not bypass credential redaction, because `redaction_is_mandatory` guards that raw-serve path regardless of passthrough state. **Exception 2 — the multi-level dispatchers (`cargo`, `dotnet`, `go`, `swift`), off a TTY**: the gate declines whenever `handler_reads_stdin` is true, and `handler_visible_args` strips a leading non-flag token for those four tools, so `skim cargo check` reduces to an EMPTY arg slice and `should_read_stdin` (`!stdin.is_terminal() && no_real_args`) returns true — in every agent harness, CI job and pipeline, i.e. exactly where the escape hatch is reached for. The command then falls through to the compressed handler, and the build family has no passthrough branch of its own (`cmd/build/mod.rs::run_parsed_command` always spawns the tool and always summarises), so `SKIM_PASSTHROUGH=1 skim cargo check` returns the compressed summary and bare `skim cargo build` has been measured returning **0 bytes**. Interactively the same command works, because a terminal stdin makes `should_read_stdin` false and the gate fires. To get raw bytes out of a build tool off a TTY, run the tool directly rather than through `skim`. Indefinite commands (`vite dev`, `jest --watch`, bare `skim vitest`) auto-pass-through live; use `skim vitest run` for a compressed one-shot.
...
- `SKIM_CACHE_DIR` — relocates **all** skim cache state: parser cache (`.json` files),
  ...
  treated as unset (falls back to `dirs::cache_dir()/skim` — `~/Library/Caches/skim` on
  macOS, `~/.cache/skim` on Linux). The path is used as-is (no `skim`
