...
When adding a new parser, register the tool in `cmd/registry.rs` (`KNOWN_SUBCOMMANDS` array) and `cmd/dispatch.rs` (`dispatch()` match arm), not in `cmd/mod.rs`. Meta-subcommands (e.g. `proxy`, `init`, `stats`, `heatmap`, `search`, `discover`, `learn`) are registered in both `KNOWN_SUBCOMMANDS` and the `is_meta_subcommand` set in `cmd/registry.rs` — they are never added to `wrapper_targets()`. The Wave-2 `skim proxy` command lives in `cmd/proxy.rs` and is dispatched as a meta-subcommand.
...
**`output::compressed_output_hint` — shared formatter**: the hint string itself is generated by `crate::output::compressed_output_hint(code: i32) -> String` (in `output/mod.rs`), not formatted inline at call sites. Both `record_and_report` (in `execution.rs`) and `emit_failure_context` (in `cmd/test/shared.rs`) call this helper. Do not construct the string `"[skim] compressed output (exit N). SKIM_PASSTHROUGH=1 for full output."` inline in new code — call `output::compressed_output_hint(code)` so the wording stays consistent across all surfaces.
...
`BuildResult::ensure_rendered(&mut self)` recomputes the `rendered` field when it is empty — needed after JSON deserialization because `rendered` is `#[serde(skip_serializing)]` and therefore absent from serialized JSON. All other canonical types (`LintResult`, `TestResult`, `GitResult`, `LogResult`, `FileResult`, `PkgResult`, `InfraResult`, `DbResult`) follow the same `new()` + `ensure_rendered()` pattern.
...
- `crates/rskim/src/cmd/dispatch.rs` — `dispatch()`, `run_raw_passthrough()`, `run_inherited_passthrough()` (inherited-stdio path for daemon/streaming commands); `spawn_status_to_code(status: io::Result<ExitStatus>) -> u8` (pure exit-code mapper: ENOENT→127, other error→1, signal-kill→1, otherwise clamp to `[0,255]`; `pub(crate)` and independently unit-tested); per-tool dispatcher helpers. Note: the Wave-2 `proxy` meta-subcommand routes to `cmd/proxy.rs::run()` — the `test_is_indefinite_command_dispatch_boundary` test asserts `!is_indefinite_command(&["proxy"])` (AC25) so the proxy startup path is never swallowed by `run_inherited_passthrough`.
