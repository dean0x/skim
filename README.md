...
- **Composite ranking** — `--weights lexical,ast,temporal` (default `0.5,0.3,0.2`) tunes the `--blast-radius` composite RRF ranking (#200); values are ratios only (not normalized), zero and non-sum-to-1 are allowed, negative/NaN/inf are rejected
