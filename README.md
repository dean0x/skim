...
- File reads (`cat`, `head`, `tail` on code files) are rewritten into direct skim reads (e.g. `cat file.ts` → `skim file.ts --mode=pseudo`); output is a structured view, not raw bytes — skim emits a one-line stderr notice whenever the served view differs from raw file contents. `head -20 file.ts` becomes `skim file.ts --mode=full --max-lines 20` (verbatim lines; one slot used by the elision marker), and `tail -5 file.ts` likewise with `--last-lines 5`; bare `head`/`tail` default to 10 lines
...
- **AST structural search** — `--ast <pattern>` filters to files containing a named structural pattern (e.g. `try-catch`, `nested-loop`, `god-function`) or a containment query (e.g. `--ast "for_statement > await_expression"`); composable with text query and `--blast-radius`. Limitations: single-node queries (no `>` separator) are rejected (#283, unigram index not yet built); combining `--ast` with `--hot`/`--cold`/`--risky` temporal sort flags is rejected (#202, merge-sort semantics not yet defined)
