...
- **`ast_coverage` in `--ast` JSON output and `--stats --json`** (#405) — The
  ...
  `skim search --stats --json` always includes `ast_coverage` regardless of `is_clean()`.

- **`skim search` coverage notice on `--build` / `--rebuild` / `--update` / `--ast` / `--stats`**
  (#405) — When files are excluded from AST indexing by the 1 MiB cap, a notice is printed
  to stderr on every explicit build, refresh, standalone `--ast` query, and `--stats` invocation.
  The notice lists the file count and per-language breakdown, and is suppressed when all
  files are within cap.  It does NOT fire on pure-lexical queries or incremental self-heals
  (D-4 cadence).
...
- **`skim search --stats` AST coverage section (text mode)** (#405) — `skim search --stats`
  ...
  after the existing stats, showing how many files are participating in AST indexing.
