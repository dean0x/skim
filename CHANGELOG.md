...
- `skim log` pending_stack is now capped at 512 frames, preventing unbounded memory growth on pathological log streams (DoS hardening). (commit ec32165)
- `skim log` JSON field values are now capped at 512 characters before storing in dedup structures, preventing memory exhaustion on large-value log lines (DoS hardening). (commit ec32165)
